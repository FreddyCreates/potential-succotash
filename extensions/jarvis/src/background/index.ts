/* ============================================================
 *  JARVIS AI — Background Service Worker v7.0
...
class JarvisEngine {
  ...
  executeChat(message: string, callback: (r: { success: boolean; message: string; agent: string; mood: string; awareness: number }) => void) {
    ...
    if (/generate (pdf|report|document)|create (pdf|report)|make (pdf|report)|pdf report/i.test(text)) {
      ...
      downloadPdf({ title: title || 'JARVIS Report', content: `Report generated by JARVIS AI v7.0 on ${new Date().toLocaleDateString()}\n\nTopic: ${raw}`, author: 'Alfredo' });
      ...
      downloadExcel({ title: title || 'JARVIS Report', author: 'Alfredo', sheets: [{ name: 'Report', columns: ['#', 'Item', 'Value', 'Notes'], rows: [[1, 'Generated by JARVIS v7.0', new Date().toLocaleDateString(), raw.substring(0, 50)]] }] }).catch(() => {});
