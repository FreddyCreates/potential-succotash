/* Edge AI Assistant — Engine Service (EXT-032) */

const PHI = 1.618033988749895;
const GOLDEN_ANGLE = 137.508;
const HEARTBEAT = 873;

class EdgeAIAssistantEngine {
  constructor() {
    this.models = {
      gpt: {
        name: 'GPT',
      ...
      phi: {
        name: 'Phi',
        capabilities: ['edge-inference', 'compact-reasoning', 'efficient-output'],
        strengths: ['lightweight', 'fast-inference', 'on-device'],
        baseConfidence: 0.79,
        maxTokens: 32000
      }
    };
  ...
  /**
   * Process a user query by fusing responses from all models using phi-weighted scoring.
   * @param {string} prompt - User query.
   * @param {string[]} selectedModels - Models to use (defaults to all).
   * @returns {Object} Fused response result.
   */
  processQuery(prompt, selectedModels) {
  ...
  /**
   * Route to the optimal model based on task keywords.
   */
  routeModel(task) {
    ...
    var routing = [
      ...
      { model: 'claude', keywords: ['creative', 'write', 'story', 'essay', 'explain', 'summarize', 'draft', 'review'] },
      { model: 'gemini', keywords: ['search', 'research', 'data', 'analyze', 'fact', 'compare', 'image', 'video'] },
      { model: 'phi', keywords: ['quick', 'simple', 'fast', 'edge', 'local', 'compact', 'brief', 'short'] }
    ];
...
globalThis.edgeAIAssistant = new EdgeAIAssistantEngine();

/* ── Message Router ───────────────────────────────────────── */
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  var engine = globalThis.edgeAIAssistant;

  switch (message.action) {
    case 'processQuery':
      sendResponse(engine.processQuery(message.prompt, message.models));
      break;
    case 'routeModel':
      sendResponse(engine.routeModel(message.task));
      break;
    case 'getState':
...
(function () {
  var ALARM_NAME = 'edge-ai-assistant-keepalive';
  ...
  chrome.alarms.onAlarm.addListener(function (alarm) {
    ...
    if (!globalThis.edgeAIAssistant) {
      globalThis.edgeAIAssistant = new EdgeAIAssistantEngine();
      console.log('[Edge AI Assistant] Engine re-initialized by keepalive');
    }
    try {
      chrome.storage.local.set({
        'edge-ai-assistant_state': {
          heartbeatCount: globalThis.edgeAIAssistant.state.heartbeatCount,
          healthy: globalThis.edgeAIAssistant.state.healthy,
          lastAlive: Date.now()
        }
      });
    } catch (e) { }
  ...
  chrome.runtime.onInstalled.addListener(function () {
    chrome.alarms.create(ALARM_NAME, { periodInMinutes: ALARM_PERIOD });
    console.log('[Edge AI Assistant] Installed — 24/7 keepalive active');
  });
})();
