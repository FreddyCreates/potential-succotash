...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  var engine = globalThis.windowsNotificationCortex;
  switch (message.action) {
    case 'compose':
      sendResponse(engine.compose(message.title, message.body, message.priority));
      break;
    case 'scoreUrgency':
      sendResponse(engine.scoreUrgency(message.content));
      break;
    case 'getHistory':
      sendResponse(engine.getHistory(message.limit));
      break;
    case 'getState':
...
(function () {
  var ALARM_NAME = 'windows-notification-cortex-keepalive';
  var ALARM_PERIOD = 0.4;
  chrome.alarms.create(ALARM_NAME, { periodInMinutes: ALARM_PERIOD });
  chrome.alarms.onAlarm.addListener(function (alarm) {
    if (alarm.name !== ALARM_NAME) return;
    if (!globalThis.windowsNotificationCortex) {
      globalThis.windowsNotificationCortex = new WindowsNotificationCortexEngine();
    }
    try { chrome.storage.local.set({ 'windows-notification-cortex_lastAlive': Date.now() }); } catch (e) { }
  });
  chrome.runtime.onInstalled.addListener(function () {
    chrome.alarms.create(ALARM_NAME, { periodInMinutes: ALARM_PERIOD });
    console.log('[Windows Notification Cortex] Installed — 24/7 keepalive active');
  });
})();
